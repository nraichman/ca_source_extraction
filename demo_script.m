...
addpath(genpath('utilities'));%Nadav: Not sure what the result of this function call is.

nam = 'demoMovie.tif';          % insert path to tiff stack here %Nadav: The default example path doesn't make sense.
sframe=1;						% user input: first frame to read (optional, default 1)
num2read=2000;					% user input: how many frames to read   (optional, default until the end)
Y = bigread2(nam,sframe,num2read);

%Y = Y - min(Y(:));
if ~isa(Y,'double');    Y = double(Y);  end         % convert to single %%Nadav: This function converts to double.
